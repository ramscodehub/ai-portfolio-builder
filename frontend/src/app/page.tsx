...
const GithubIcon = () => ( <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" role="img" aria-hidden="true"> <path d="M12 2C6.477 2 2 6.477 2 12c0 4.418 2.865 8.165 6.839 9.489.5.092.682-.218.682-.482 0-.237-.009-1.024-.014-1.862-2.782.603-3.369-1.21-3.369-1.21-.454-1.156-1.11-1.464-1.11-1.464-.908-.62.069-.608.069-.608 1.004.071 1.532 1.03 1.532 1.03.891 1.529 2.34 1.087 2.91.831.091-.646.349-1.087.635-1.338-2.22-.253-4.555-1.113-4.555-4.949 0-1.092.39-1.985 1.03-2.685-.104-.253-.448-1.272.098-2.648 0 0 .84-.269 2.75 1.025A9.547 9.547 0 0 1 12 6.836c.85.004 1.705.114 2.504.336 1.909-1.294 2.748-1.025 2.748-1.025.548 1.376.204 2.395.1 2.648.64.7 1.028 1.593 1.028 2.685 0 3.848-2.337 4.695-4.566 4.942.359.31.678.92.678 1.852 0 1.336-.012 2.415-.012 2.741 0 .267.18.577.688.482A10.001 10.001 0 0 0 22 12c0-5.523-4.477-10-10-10Z" /> </svg> );
const LinkedInIcon = () => ( <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" role="img" aria-hidden="true"> <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" /> </svg> );
...
export default function HomePage() {
  ...
  // NEW: State to hold the final, clickable link
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true); 
    setError(null); 
    setFinalLink(null); // Reset previous link on new submission
    setStatusMessage("Building your portfolio... This may take 2-3 minutes.");
    setShowUnoLink(true);
    ...
    try {
        ...
        // UPDATED: Instead of opening a window, save the link to state
        if (data.view_link) {
            setFinalLink(data.view_link);
        } else {
  ...
  return (
    <div className={styles.mainContainer}>
      ...
      {/* --- UPDATED Status, Error, and Result Link Area --- */}
      {(isLoading || statusMessage || error || finalLink) && ( // Show container if any message OR a final link exists
        <div className={styles.statusMessageContainer}>
          ...
          {/* NEW: Conditionally render the "View Portfolio" button */}
          {!isLoading && finalLink && (
            <a 
              href={finalLink}
              target="_blank"
              rel="noopener noreferrer"
              className={styles.resultLinkButton}
            >
              View Your Portfolio ✨
            </a>
          )}
